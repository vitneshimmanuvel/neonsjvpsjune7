...
export async function updateEntry(registerId: number, entryId: number, cells: Record<string, string>): Promise<Entry> {
  ...
  // Asynchronous activity logging (non-blocking for UI save speed)
  logAction(reg.businessId, 'Edit Row', details, { registerId, registerName: reg.name, entryId }).catch(console.error);

  // Sync linked columns concurrently
...
async function _syncLinkedColumn(targetRegisterId: number, targetColumnId: number, sourceEntryId: number, rowNumber: number, value: string) {
  return runQueuedMutation(targetRegisterId, async () => {
    ...
    if (targetEntry) {
      ...
      // Fast single-entry targeted update (50ms) instead of re-uploading the entire 700-row table
